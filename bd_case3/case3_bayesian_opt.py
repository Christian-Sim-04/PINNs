...
def run_bayesian_optimisation(data, epochs_per_trial, n_trials, device):
    #unpack the data
    x1_pde_torch, x2_pde_torch, xmin, xmax, q0 = data['x1'], data['x2'], data['xmin'], data['xmax'], data['q0']

    def objective(trial):
        pde_weight = trial.suggest_float('pde_weight', 1e-3, 1000, log=True)#log scale search
        bc_weight = trial.suggest_float('bc_weight', 1e-6, 100, log=True)
        if_weight = trial.suggest_float('if_weight', 1e-6, 100, log=True) #individual weights now applied
