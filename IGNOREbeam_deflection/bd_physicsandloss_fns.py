...
def bc_loss(model, xmin, xmax):
    ...
    loss_B = ((EI3 * w3_xx)**2).mean() + ((EI3 * w3_xxx)**2).mean()  #w3_xx_denorm if denormalising
...
def interface_loss(model, xmin, xmax):
    ...
    epsilon = 1e-5
    #x2_left = torch.full((1,1), normalise([2.0 - 1e-5].reshape(-1,1)), dtype=torch.float32, requires_grad=True)
    #x2_right = torch.full((1,1), normalise([2.0 + 1e-5].reshape(-1,1)), dtype=torch.float32, requires_grad=True)
