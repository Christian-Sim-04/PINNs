...
def run_bayesian_optimisation(data, epochs_per_trial, n_trials, device):
    #unpack the data
    x_pde_torch, xmin, xmax, q0 = data['x_pde'], data['xmin'], data['xmax'], data['q0']

    def objective(trial):
        pde_weight = trial.suggest_float('pde_weight', 1e-1, 1000, log=True)#log scale search
        bc_weight = trial.suggest_float('bc_weight', 1e-3, 100, log=True)
        ...
        for ep in range(epochs_per_trial):
            optimizer.zero_grad()
            loss_residual = pde_loss(model_single_beam, x_pde_torch, xmin, xmax, q0)
